...
final class InternalProjectSettingsTests: XCTestCase {
    ...
    func test_containsNoInternalProjectSettings_returnsClean2() {
        do {
            let testData = Bundle.test.testData(.good)
            let errorReporter = ErrorReporter(pbxprojPath: testData, reportKind: .error)
            let project = try Project(testData, errorReporter: errorReporter)
            ...
            XCTAssertEqual(checkForInternalProjectSettings2(xcodeproj, path, project.projectText, errorReporter: errorReporter), EX_OK)
        } catch {
            print(error.localizedDescription)
            XCTFail("Failed to initialise test")
        }
    ...
    func test_containsInternalProjectSettings_returnsError2() {
        do {
            let testData = Bundle.test.testData()
            let errorReporter = ErrorReporter(pbxprojPath: testData, reportKind: .error)
            let project = try Project(testData, errorReporter: errorReporter)
            let path = Path(testData)
            let xcodeproj = try XcodeProj(path: path)
            XCTAssertEqual(checkForInternalProjectSettings2(xcodeproj, path, project.projectText, errorReporter: errorReporter), EX_SOFTWARE)
        } catch {
            print(error.localizedDescription)
            XCTFail("Failed to initialize test")
        }
